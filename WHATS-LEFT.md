...
**What's still needed beyond that:** Even once confirmed installed, their logs don't automatically reach Log Analytics; set up DCR only collects *Windows* Event Logs. Getting Zeek/Suricata data into the same pipeline needs a second, Linux-specific Data Collection Rule and the Linux version of Azure Monitor Agent — genuinely new Terraform work and not just a leftover step.
...
**What building it takes:** In the Azure Portal, search "Microsoft Sentinel" → Create → point it at the existing workspace. Genuinely a few clicks deliberately left undone because turning it on without immediately building real analytics rules behind it isn't especially useful, and that's a session in itself.
...
**Why it's a bigger separate step:** Defender products need licensing/enablement at the tenant level, and endpoint protection specifically needs its own agent installed per VM which is a genuinely separate onboarding flow from anything built tonight.
...
None of it is hard individually but sequential. Each piece unlocks the next.
