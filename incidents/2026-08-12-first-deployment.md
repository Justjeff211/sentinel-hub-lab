...
Run `terraform apply` for the first time against the network and identity Terraform, then promote DC1 to a domain controller and domain-join AADCONNECT - the first real infrastructure this project has stood up and not just designed.
...
- **DC1 promoted** via `Install-ADDSForest` and is verified with `Get-ADDomain`, `Get-Service DNS,NTDS,Netlogon` (all running), and `Get-ADDomainController -Filter *` (confirmed IP matches Terraform's static assignment).
- **AADCONNECT domain-joined** to `sentinelhub.local` and is verified with `Get-CimInstance Win32_ComputerSystem` (`PartOfDomain: True`) and `nltest /sc_query` (`NERR_Success`, trusted DC correctly identified).

Bastion login also failed intermittently on both VMs even with a freshly-reset password. This I managed to fix by using `.\Justjeff` (explicit local-account prefix) instead of just `Justjeff`.
